...
RSpec.describe Users::OauthFinder, type: :service do
  ...
  let(:auth) { OmniAuth::AuthHash.new(provider: 'github', uid: '123123') }
  subject { Users::OauthFinder.new(auth) }

  context 'user has authorization' do
    it 'returns the user' do
      user.authorizations.create(provider: 'github', uid: '123123')
  ...
  context 'user has not authorization' do
    context 'user already exists' do
      let(:auth) { OmniAuth::AuthHash.new(provider: 'github', uid: '123123', info: { email: user.email }) }
